...
export async function insertUser(db, { email, originalPassword, bio = '', name, profilePicture, username, role, masterAccountId }) {
  ...
  if (!existingUser) {
    const result = await db.collection('users').updateOne({ email: email },{ $set: { ...user, password }}, { upsert: true });
  } else {
    return 'user exists';
  }
